...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not self.doubled_damage:
            self.damage *= 2
            self.doubled_damage = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        place, entrances_traversed = self.place, 0
        while not place.is_hive:
            if self.lower_bound <= entrances_traversed <= self.upper_bound:
                if target_bee := random_bee(place.bees):
                    return target_bee
            place, entrances_traversed = place.entrance, entrances_traversed + 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in bees:
            bee.reduce_health(amount + (self.damage if self.health <= 0 else 0))


class WallAnt(Ant):
    """WallAnt does nothing but has beefy health."""
...
class HungryAnt(Ant):
    """HungryAnt eats a random Bee in its Place, taking 3 turns to get it down."""
    ...
    def action(self, gamestate):
        """Eat a Bee in same Place as HungryAnt."""
        if self.turns_to_chew:
            self.turns_to_chew -= 1
        elif eaten_bee := random_bee(self.place.bees):
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and deals damage to Bees in its
    Place."""
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        def slower(target, slow_time=5):
            target.slow_time = slow_time

            def wrapper(gamestate):
                if (
                    gamestate.time % 2 == 0 and target.slow_time
                ) or not target.slow_time:
                    Bee.action(target, gamestate)
                target.slow_time -= 1 if target.slow_time else 0

            return wrapper

        if target is not None:
            target.action = slower(target)
...
class LaserAnt(ThrowerAnt):
    ...
    food_cost = 10
    damage = 2
    distance_damage_attenuation = 0.25
    insect_hit_damage_attenuation = 0.0625
    implemented = True
    ...
    def insects_in_front(self):
        insect_dict, place, distance = {}, self.place, 0
        while not place.is_hive:
            if place.ant:
                if place.ant is not self:
                    insect_dict[place.ant] = distance
                if place.ant.is_container and place.ant.ant_contained is not self:
                    insect_dict[place.ant.ant_contained] = distance
            for bee in place.bees:
                insect_dict[bee] = distance
    ...
    def calculate_damage(self, distance):
        if damage := max(
            0, self.damage - (self.distance_damage_attenuation * distance)
        ):
            self.damage -= self.insect_hit_damage_attenuation
        return damage
